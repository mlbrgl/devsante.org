  ...
  transformItems: function (items) {
    return items.map(function (item) {
      ...
      if (item._distinctSeqID !== 0) {
        ...
        updatedItem.datetime = get_locale_date_string_fallback(
          item.datetime * 1000
        );
      }
...
function clear_search(event) {
  ...
  var search_input = document.querySelectorAll(
...
if (
  !Cookies.get("hide-cookie-consent-until") ||
  Date.now() > Cookies.get("hide-cookie-consent-until")
) {
